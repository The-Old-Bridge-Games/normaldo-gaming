...
final class CloneAttack extends Attack with Effects {
  ...
  Sprite get _cloneSprite {
    if (cloneSprites.length == 1) {
      ...
      return cloneSprites.toList()[Random().nextInt(cloneSprites.length)];
    }
  }
    ...
    _wavesTimer = TimerComponent(
      ...
      onTick: () {
        if (_tick >= wavesAmount / 3 && !hasNormaldoAttack) {
          ...
          final punch = FistPunch();
          boss.game.grid.add(
            punch
              ..position =
                  Vector2(boss.position.x - boss.size.x, boss.position.y)
  ...
  void _startWave(Boss boss) {
    final startPositions = List<double>.from(boss.game.grid.linesCentersY);
    startPositions.removeAt(Random().nextInt(startPositions.length));
    final removeOneMore = Roller<bool>([(false, 2), (true, 1)]).roll();
    if (removeOneMore) {
      startPositions.removeAt(Random().nextInt(startPositions.length));
    }
